...
- `/model` is the tool: collapsible sections (You / Superannuation / Spending / Bank/investment accounts / Income / Assumptions) feeding a **pinned chart** that updates live. Bank/investment accounts take a single **return** and are taxed on the whole of it each year (conservative — a growth investment's unrealised gains would really be deferred, but taxing the lot keeps the input simple). Includes an **"Age now"** field (drives the accumulation phase), an **"Include Age Pension"** toggle, a frequency-based **spending breakdown**, and a **"Reset all values"** button.
...
- `assets.ts` — `Asset` + the `ITaxable` capability (`assessableIncomeOn`). `Super` is structurally tax-free (not `ITaxable`); `BankAccount` is taxable on its **whole return** each year (conservative — over-taxes a growth investment whose unrealised gains wouldn't really be taxed, but keeps the input to a single "Return" field; CGT-on-sale isn't modelled). `isTaxable` guard.
...
- `projection.ts` — `project(assets, assumptions, scenario)`: runs an **accumulation phase** (before `retireAge`: contributions in, 15%-earnings-taxed growth, no spending) then **drawdown** (income offset, tax, Age Pension, spend; the bad-case crash is timed to retirement). Drawdown spends the **ATO minimum drawdown first**, then the taxed accounts, and only pays super **above** its minimum once those are empty; the forced excess beyond the year's need goes to a running **`setAside`** bucket, and the net draw is floored at zero (no surplus reinvestment) — see design-philosophy point 7. All the policies (tax, pension, income, contributions) are **injected callbacks** — the engine knows nothing about SAPTO or filing.

**What's modelled:** super (tax-free in pension phase; 15%-taxed earnings in accumulation), bank/investment accounts (taxed each year on their full return — conservative, over-taxing a growth investment for simplicity), retirement income (work/rent, indexed or fixed), **salary contributions into super** (SG % + salary sacrifice, 15% contributions tax), single or couple, the **Age Pension** (assets test **and** income test with deeming, paying the lower; phasing in as assets fall), average vs bad-case (sequence-risk) scenarios, and a deliberately-simple **drawdown** (ATO minimum spent first, then the taxed accounts, super above its minimum only as a last resort; forced excess set aside; no surplus reinvestment — see design-philosophy point 7) to a plan-to age with run-out detection.
...
**7. Drawdown is deliberately simple — a philosophical choice, not a "correct" cash-flow engine.** The vision's lowest-tax-first waterfall with surplus reinvestment (VISION.md) is the someday-ideal; what's _built_ is a blunter model picked so every year reconciles in the calc panel. It **used to be uniformly conservative** ("always err toward the user being worse off, never flattered"); the draw-order fix below deliberately traded some of that for realism, because the old order modelled a choice no real retiree would make. The remaining simplifications still lean conservative:

- **The minimum drawdown funds the year first, then the taxed accounts, and super pays above its minimum only as a last resort.** The ATO minimum leaves super whether it's wanted or not, so it's spent before anything else; the rest of the need comes from bank/investment accounts, leaving the tax-free fund to compound. This **replaced an earlier super-first rule** (spend super down, touch other accounts only once it's exhausted), which was conservative but indefensible: nobody voluntarily drains the 0%-tax wrapper while holding taxed cash. The cost of the change is that non-super accounts are no longer just "opening × growth" in a normal year — the pro-rata sliver the old order avoided now shows up in the calc panel, and `Calculations.svelte` mirrors the new order so every year still reconciles.
...
- **This is a philosophical decision, not a claim of correctness.** It is explicitly _not_ a cash-flow optimiser; it's a defensible, transparent simplification. The one remaining un-conservative direction is the draw order (preserving super now flatters slightly vs. the old super-first rule) — accepted, because the old order was conservative by modelling something nobody does, and a distortion you have to apologise for isn't a safety margin. The still-unfaithful piece is the **set-aside bucket**: reinvesting the excess into a taxable account and drawing it as a last resort is deferred as a **Pro-tier** feature.
...
- **Super fees are a separate input** (default **0.85%/yr**, admin + investment), subtracted from the super return before the projection — matching how Moneysmart treats fees. So the super **return is entered _before_ fees**. Defaults are calibrated to reconcile with Moneysmart's **Balanced** option: **8%** return − 0.85% fees, then the 15% accumulation earnings tax, ≈ **6.1% net** (the figure Moneysmart quotes). Bank/investment accounts still have **no fees field** — their return is entered net of fees, so a gross figure there over-projects.
- **Inflation defaults to 3%** — the top of the RBA 2–3% band, chosen over the 2.5% midpoint because retiree costs (health, aged care, rates) outrun headline CPI and a lower rate flatters the plan. Note Moneysmart deflates at **3.7%** (2.5% CPI + 1.2% rising living standards), so even at 3% our today's-dollars balances read a touch higher than Moneysmart's — a deliberate, still-conservative-enough choice; a dedicated "spend grows above CPI" knob is the future refinement.
...
6. **Injected scenario / drawdown strategies — when the second case arrives.** Today the engine hardcodes the crash/recovery path and the minimum-first-then-taxed-accounts drawdown order. The seam is obvious; extracting it early would be premature abstraction. (Related Pro-tier item: a _faithful_ excess-handling model — reinvest the set-aside bucket in a taxable account and draw it as a last resort — instead of the current park-it-and-forget bucket.)
